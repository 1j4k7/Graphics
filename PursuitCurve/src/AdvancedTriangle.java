...
public class AdvancedTriangle extends JFrame {
	...
	public static void main(String[] args) {
		...
		frame.setSize(600, 700);//set the size of the frame
...
class NewPanel extends JPanel {
	 ...
	 */
	void initgr() {
		maxX = 200;//set maxX to the width of the panel - 1
		maxY = 200;//set maxY to the height of the panel - 1
	...
	protected void paintComponent(Graphics g) {
		...
		while (yCenter + sideHalf <= getHeight()) {
			xCenter = maxX / 2;
			while (xCenter + sideHalf <= getWidth()) {
				g.drawImage(img, iX((float) (xCenter - 20)), iY((float)(yCenter - 20)), null);
